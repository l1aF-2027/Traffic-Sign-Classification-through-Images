...
\[
\min_{\mathbf{w}, b,\xi_i} (\frac{1}{2} \|\mathbf{w}\|^2 \quad  + C\sum{\xi_i})\text{với điều kiện} \quad y_i (\mathbf{w} \cdot \mathbf{x}_i + b) \geq 1 - \xi_i \quad \forall i, \xi_i \req 0
\quad \text{\cite{boser1992}}
\]
Trong đó:
\begin{itemize}
    ...
    \item \(y_i\) là nhãn của điểm dữ liệu (\(y_i = \pm 1\)).
    \item \(\mathbf{x}_i\) là điểm dữ liệu.
    \item \(\xi_i\) là biến bù của điểmm dữ liệu \(x_i\).
    \item \(C\) là tham số điều chỉnh mức phạt đối với các lỗi phân loại.
\end{itemize}
...
\begin{itemize}
    ...
    \]
    \begin{itemize}
        \item Với \(\gamma\) là hệ số quyết định mức độ ảnh hưởng của một điểm dữ liệu khác.
...
\textbf{Khái niệm:} Precision đo lường tỷ lệ mẫu dương dự đoán đúng trong số các mẫu được dự đoán là dương.  
\textbf{Công thức:}  
\[
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\textwidth]{classification_report_knn.png}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\textwidth]{classification_report_svm.png}
...
\begin{itemize}
    ...
    \item Đối với KNN: có thể điều chỉnh n\_neighbors, weights, metrics và leaf\_size
...
\begin{center}
    \includegraphics[width=0.35\textwidth]{link_web.png}
\end{center}
